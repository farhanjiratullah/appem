        ...
        <tbody>
          @foreach ($data_akunMasyarakat as $akun)
              <tr class="text-center">
                  ...
                  <td>{{ $akun->telp }}</td>
                  <td>
                    <form action="{{ route('petugas.destroyakunmasyarakat', $akun->nik) }}" method="post" id="delete{{ $akun->nik }}">
                      @csrf
                      @method('delete')

                      <button type="button" class="btn btn-danger btn-sm" onclick="deleteData({{ $akun->nik }})">
